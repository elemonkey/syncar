...
class Importer(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    name: Mapped[str] = mapped_column(
        SQLEnum(ImporterType), unique=True, nullable=False, index=True
    )
    ...
    categories: Mapped[List["Category"]] = relationship(
        "Category", back_populates="importer", cascade="all, delete-orphan"
    )
    products: Mapped[List["Product"]] = relationship(
        "Product", back_populates="importer", cascade="all, delete-orphan"
    )
    jobs: Mapped[List["ImportJob"]] = relationship(
        "ImportJob", back_populates="importer", cascade="all, delete-orphan"
    )
    config: Mapped[Optional["ImporterConfig"]] = relationship(
        "ImporterConfig",
...
class Category(Base):
    ...
    importer_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("importers.id"), nullable=False
    )
    ...
    )  # Para guardar selección del usuario
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now()
    )
    updated_at: Mapped[datetime] = mapped_column(
...
class ImporterConfig(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    importer_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("importers.id"), unique=True, nullable=False
    )
    ...
    products_per_category: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True, default=None
    )  # None = sin límite, scrapea todos
